...
class Parser:
    ...
    def _parse_sup(self, sql):
        ...
        self.colSet.add(
            sql["col_set"].index(sql["names"][sql["sql"]["orderBy"][1][0][1][1]])
        )
        result.append(
            C(sql["col_set"].index(sql["names"][sql["sql"]["orderBy"][1][0][1][1]]))
        )
    ...
    def _parse_filter(self, sql):
        ...
        if sql["sql"]["where"] != []:
            ...
            if len(sql["sql"]["where"]) == 1:
                result.extend(
                    self.parse_one_condition(sql["sql"]["where"][0], sql["names"], sql)
                )
            elif len(sql["sql"]["where"]) == 3:
                ...
                result.extend(
                    self.parse_one_condition(sql["sql"]["where"][2], sql["names"], sql)
                )
            else:
                if sql["sql"]["where"][1] == "and" and sql["sql"]["where"][3] == "and":
                    result.append(Filter(0))
                    result.extend(
                        self.parse_one_condition(
                            sql["sql"]["where"][0], sql["names"], sql
                        )
                    )
                    result.append(Filter(0))
                    result.extend(
                        self.parse_one_condition(
                            sql["sql"]["where"][2], sql["names"], sql
                        )
                    )
                    result.extend(
                        self.parse_one_condition(
                            sql["sql"]["where"][4], sql["names"], sql
                        )
                    )
        ...
        if sql["sql"]["having"] != []:
            result.extend(
                self.parse_one_condition(sql["sql"]["having"][0], sql["names"], sql)
            )
        return result, None
    ...
    def _parse_order(self, sql):
        ...
        if (
            "order" not in sql["query_toks_no_value"]
            or "by" not in sql["query_toks_no_value"]
        ):
            return result, None
